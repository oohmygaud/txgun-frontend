...
function * startLogin (action) {
  try {
    ...
    } else yield put({ type: 'LOGIN_DENIED' })
...
function * startLogout () {
  try {
    ...
    if (response.status == 200) yield put({ type: 'LOGOUT_SUCCEEDED' })
  } catch (e) {
...
function * getProfile () {
  try {
    ...
    yield put({
      type: 'GET_PROFILE_SUCCEEDED', user: response.data, username: response.data.username, user_id: response.data.id
    })
  } catch (e) {
...
function * register (action) {
  try {
    ...
    if (response.status == 201) {
      ...
      yield put({ type: 'LOGOUT' })
      yield put(push('/login'))
    } else yield put({ type: 'REGISTRATION_DENIED', errors: response.data })
  } catch (e) {
