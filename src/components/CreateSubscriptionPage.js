...
import { editSubscription, createSubscription, getABI, loadSubscriptionDetail, archiveSubscription, unarchiveSubscription } from '../store/actions/subscriptions'
...
export class CreateSubscription extends React.Component {
    ...
    OnChangeDefaultEmailSwitch (e) {
      ...
      if (e.target.checked) { this.setState({ notify_email: this.props.user.email }) }
    }

    OnChangeDefaultURLSwitch (e) {
      this.setState({ default_url: e.target.checked })
      if (e.target.checked) { this.setState({ notify_url: this.props.user.default_notify_url }) }
    }

    OnChangeSpecificContractCalls (e) {
      this.setState({ specific_contract_calls: e.target.checked })
      if (e.target.checked) { this.props.getABI(this.state.watched_address) }
    }
    ...
    renderNotifyUrlField () {
      const onChangeRealtimeWebhooks = (e) => {
        const update = { realtime_webhooks: e.target.checked }
        if (!e.target.checked) { update.notify_url = '' }
    ...
    componentDidMount () {
      ...
      if (this.props.match.params.id) { this.props.loadSubscriptionDetail(this.props.match.params.id) }
    }
    ...
    render () {
      ...
      if (!this.props.subscription && this.props.match.params.id) { return <Typography>Loading...</Typography> }
            ...
            {this.props.subscription
              ? !this.props.subscription.archived_at
                ? <Button
                  color="secondary"
                  style={{ marginTop: '1em' }}
                  onClick={() => this.props.archive(this.props.subscription.id)} >
                  <ArchiveIcon />Archive
                </Button>
                : <Button color="secondary" onClick={() => this.props.unarchive(this.props.subscription.id)} >
